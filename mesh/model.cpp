...
void Model::render(Shader &shader) const
{
    ...
    // 新增：传递骨骼变换矩阵到Shader
    if (animator_ && animator_->GetCurrentAnimation())
    {                                       // 仅当有Animator且正在播放动画时才传递
                                            // if(false)   { //for debug
    ...
    else
    {
        ...
        // const auto &boneMatrices = animator_->GetFinalBoneMatrices();//I'will come here later...
        // for (int i = 0; i < boneMatrices.size(); ++i)
        // {
        //     shader.setMat4("finalBoneMatrices[" + std::to_string(i) + "]", Eigen::Matrix4f::Identity());
        // }
