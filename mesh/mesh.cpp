...
Mesh::Mesh(const std::string& name, 
    ...
    : name_(name),
      vertices_(vertices), // 将传入的顶点数据复制到成员变量
      indices_(indices),   // 将传入的索引数据复制到成员变量
...
void Mesh::render(Shader &shader) const {
    ...
    VAO_->bind();
    glDrawElements(GL_TRIANGLES, indices_.size(), GL_UNSIGNED_INT, 0); // 使用成员变量indices_
    VAO_->unbind();
}
